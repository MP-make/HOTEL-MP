...
// La carpeta 'img' está dentro de 'Fronted/Public'
const uploadDir = path.join(__dirname, '..', 'Fronted', 'Public', 'img');
...
try {
    ...
    VALUES($1, $2, $3, (SELECT id_rol FROM public.roles WHERE nombre = 'cliente'))
    RETURNING *;
    `;
    const result = await pool.query(queryText, [nombre, email, passwordHash]);
 ...
 */
app.get("/api/encargado/reservas", async (req, res) => {
try {
    const queryText = `
    SELECT
        r.id_reserva,
 ...
 */
app.post("/api/cliente/reservas", async (req, res) => {
